...
def find_firmware_jumptable(boot_path):
    ...
    for off in range(0, len(data) - JT_BYTES, 4):
        if (
            data[off + 0xA00 : off + 0xA04] == sig_a
            and data[off + 0xA0C : off + 0xA10] == sig_b
        ):
            words = [
                struct.unpack_from("<I", data, off + i * 4)[0]
                for i in range(JT_BYTES // 4)
            ]
...
def extract_offset(code, insns):
    ...
    for ins in insns:
        ...
        if mn == "ldr" and len(ops) == 2 and ops[0].type == capstone.arm.ARM_OP_REG:
            ...
            if src.type == capstone.arm.ARM_OP_MEM:
                ...
                if mem.base == capstone.arm.ARM_REG_PC and mem.index == 0:
                    ...
                    state[dst.reg] = (
                        struct.unpack_from("<I", code, lit)[0]
                        if lit + 4 <= len(code)
                        else None
                    )
                elif mem.index == 0:
        ...
        if (
            mn == "mov"
            and len(ops) == 2
            and ops[0].type == capstone.arm.ARM_OP_REG
            and ops[1].type == capstone.arm.ARM_OP_REG
        ):
        ...
        if mn in ("pop", "ldm", "ldmia", "ldmdb", "ldmib"):
            ...
            continue
        if (
            mn in ("push", "stm", "stmia", "stmdb", "stmib")
            or mn.startswith("str")
            or mn in READ_ONLY
        ):
            continue
...
def load_sdk_functions():
    ...
    subprocess.run(
        ["ar", "x", abspath(join(DATA, "libv5rt.a"))],
        cwd=tmp,
        check=True,
        capture_output=True,
    )
        ...
        with open(obj, "rb") as f:
                ...
                if isinstance(section, RelocationSection):
                    relocs_by_sec.setdefault(section["sh_info"], []).extend(
                        (
                            r["r_offset"],
                            r["r_info_type"],
                            symtab.get_symbol(r["r_info_sym"]).name,  # type:ignore
                        )
            ...
            for section in elf.iter_sections():
                ...
                for sym in section.iter_symbols():
                    ...
                    code = sec.data()[
                        sym.entry["st_value"] : sym.entry["st_value"] + size
                    ]
                    rels = [
                        (ro, rt, sn)
                        for ro, rt, sn in relocs_by_sec.get(si, [])
...
def main():
    ...
    print(
        "firmware jumptable: base=0x%x %d bytes, %d/%d slots nonzero"
        % (base, JT_BYTES, n_used, len(words))
    )
    ...
    print(
        "archive functions: %d, resolved: %d, unresolved: %d (%d glue, %d pure)"
        % (
            len(sdk),
            len(resolved),
            len(unresolved),
            len(glue),
            len(unresolved) - len(glue),
        )
    )
    api_unresolved = [n for n in unresolved if n not in glue and n.startswith("vex")]
    if api_unresolved:
        print(
            "unresolved vex* API (no jumptable dispatch): %s"
            % ", ".join(sorted(api_unresolved))
        )
    if glue:
        print(
            "glue (real logic + tail call; reimplement in codegen): %d, e.g."
            % len(glue)
        )
